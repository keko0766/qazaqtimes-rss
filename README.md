...
Default local AI model — `gpt-oss:20b`.

Бірінші рет модель жүктелуі ұзақ болуы мүмкін. Прогресс GUI-де және мына файлдарда көрінеді:
...
AI дайын болмаса немесе сапа тексерісінен өтпесе, app резерв шаблонмен мақала жасайды. Дайын болған соң келесі генерацияда Ollama автоматты қолданылады.

Модель `.env` арқылы беріледі:
...
Нәтиже бүгінгі `latest/` папкаға жазылады. Ollama setup журналы:
